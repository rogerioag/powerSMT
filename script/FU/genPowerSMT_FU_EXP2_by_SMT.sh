...
do
  ...
  do
    ...
    FILE_OUT_PUT=${FILE_OUT_PUT_BASE}_SMT_${numProcessos}_MOD_${iModulo}.${SCRIPT_EXT}
  		    	...
  		    	do
					...
					do
			  			...
			  			TEST_OPTIONS="${MODULES_CONFIG} ${CACHE_IL1} ${FU_CONFIG}"
  		      			...
  		      			# Redirecting outputs.  		      
  		      			REDIR_SIM="-redir:sim ${OUTPUT_DIR}/proc_${numProcessos}__mod_${iModulo}__nbank_${il1_nBanks}__benchs_${iBenchmark}_${fIndex}__csize_${cacheSize}__bsize_${blockSize}__assoc_${assoc}__repl_${repl}__ialu_${numFU}__imult_${numFU}__fpalu_${numFU}__fpmult_${numFU}__memport_${numFU}_sim.out"
    ...
    echo "echo \" End Execution SMT-${numProcessos} with MOD-${iModulo}\"" >> ${FILE_OUT_PUT}
    echo "echo \"End Execution SMT-${numProcessos}\"" >> ${FILE_OUT_PUT}
    let "iModulo*=2"
  done
done
