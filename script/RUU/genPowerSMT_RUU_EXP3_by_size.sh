	    ...
	    do
			for assoc in 1
  		    do
				...
				for numFU in 32
				do
				        for ruuType in distributed
					do 
						for ruuTotalSize in 256 128 64 32 16 
						do
  		    					# LSQ Size is half ruuSize.
							ruuSize=$ruuTotalSize
								
							if [ ${ruuType} == "shared" ]
				    			then ruuSize=$[$ruuTotalSize / $numProcessos]
    							else ruuSize=$ruuTotalSize
    							fi

  		      					lsqSize=$[$ruuSize / 2]
  		    					  		    					
  		    					FILE_OUT_PUT=${FILE_OUT_PUT_BASE}_RUU_${ruuType}_RUU_SIZE_${ruuSize}.${SCRIPT_EXT}
  				  			...
  				  			let "quantSimulacoes+=1"
  		    	  				...
  		    	  				echo "Generating Command Line [${quantSimulacoes}]: (proc:${numProcessos}, mod:${iModulo}, nbank:${il1_nBanks}, benchs:[${iBenchmark}, ${fIndex}], csize:${cacheSize}Kb, bsize:${blockSize} bytes, assoc:${assoc}, repl:${repl}, ialu:${numFU}, imult:${numFU}, fpalu:${numFU}, fpmult:${numFU}, memport:${numFU}, ruulsq:type:${ruuType}, ruu:size:${ruuSize}, lsq:size ${lsqSize})."
  		    	  				echo "echo \"  Running config [${quantSimulacoes}]: (proc:${numProcessos}, mod:${iModulo}, nbank:${il1_nBanks}, benchs:[${iBenchmark}, ${fIndex}], csize:${cacheSize}Kb, bsize:${blockSize} bytes, assoc:${assoc}, repl:${repl}, ialu:${numFU}, imult:${numFU}, fpalu:${numFU}, fpmult:${numFU}, memport:${numFU}, ruulsq:type:${ruuType}, ruu:size:${ruuSize}, lsq:size ${lsqSize}).\"" >> ${FILE_OUT_PUT}
								...
								# FUs configuration.
  		      					...
  		      					# RUU-LSQ configuration.
	  		      				RUU_LSQ_CONFIG="-ruulsq:type ${ruuType} -ruu:size ${ruuSize} -lsq:size ${lsqSize}"
  		      			
				  			# Specific Test Options.
				  			TEST_OPTIONS="${MODULES_CONFIG} ${CACHE_IL1} ${FU_CONFIG} ${RUU_LSQ_CONFIG}"
				  			
				  			# Nome arquivo Saida Sim.
                                                        NOME_ARQUIVO="${OUTPUT_DIR}/proc_${numProcessos}__mod_${iModulo}__nbank_${il1_nBanks}__benchs_${iBenchmark}_${fIndex}__csize_${cacheSize}__bsize_${blockSize}__assoc_${assoc}__repl_${repl}__ialu_${numFU}__imult_${numFU}__fpalu_${numFU}__fpmult_${numFU}__memport_${numFU}__ruulsq_type_${ruuType}__ruu_size_${ruuSize}__lsq_size_${lsqSize}_sim.out"
  		      
	  		      				# Redirecting outputs.  		      
			      				REDIR_SIM="-redir:sim ${NOME_ARQUIVO}"
				  			REDIR_PROG="-redir:prog ${OUTPUT_DIR}/proc_${numProcessos}__mod_${iModulo}__nbank_${il1_nBanks}__benchs_${iBenchmark}_${fIndex}__csize_${cacheSize}__bsize_${blockSize}__assoc_${assoc}__repl_${repl}__ialu_${numFU}__imult_${numFU}__fpalu_${numFU}__fpmult_${numFU}__memport_${numFU}__ruulsq_type_${ruuType}__ruu_size_${ruuSize}__lsq_size_${lsqSize}_prog.out"
			  		
				  			# CACTI outs in stdout.
				  			REDIR_STDOUT="> ${OUTPUT_DIR}/proc_${numProcessos}__mod_${iModulo}__nbank_${il1_nBanks}__benchs_${iBenchmark}_${fIndex}__csize_${cacheSize}__bsize_${blockSize}__assoc_${assoc}__repl_${repl}__ialu_${numFU}__imult_${numFU}__fpalu_${numFU}__fpmult_${numFU}__memport_${numFU}__ruulsq_type_${ruuType}__ruu_size_${ruuSize}__lsq_size_${lsqSize}_stdout.out"
...
NOME_ARQ_BATCH="execBatch_RUU_by_ruu_size.sh"
