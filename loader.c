             ...
             int sn)                    /* slot number */
{
  ...
  if (eio_valid(fname)){
    ...
    if (sim_chkpt_fname != NULL) {
    	...
    	if (!eio_valid(sim_chkpt_fname))
    		fatal(sn,"file `%s' does not appear to be a checkpoint file", sim_chkpt_fname);
  ...
  /* finally, predecode the text segment... */
  {
    ...
    for (addr=ld_text_base[sn]; addr < (ld_text_base[sn]+ld_text_size[sn]); addr += sizeof(md_inst_t))
    {
