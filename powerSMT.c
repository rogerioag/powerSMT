...
/* The variable "errno", from "include <errno.h>", is just only and because there 
 * are many slots and each one can cause an error, this structure was created in 
 * order to keep the integrity of each error from different slot. See sim_init 
 * for initialization */
...
/* number of independet i-cache modules with independent bus and that can be fetched simultaneously. 
 * The "process_num" must be a multiple of "il1_modules_num". Each i-cache module serves a different 
 * pool of n SLOTS such that n = IL1_MODULE_WIDTH.
 */
...
/* simulated registers: one independent bank for each pipeline of the SMT processor. 
 * See sim_init for initialization */

static struct regs_t *regs[MAX_SLOTS] = { NULL };

/* simulated memory, one independet memory structure for each pipeline of the SMT processor - distributed memory. 
 * Normalmente, a inicializacao abaixo eh dependente de compilador. 
 * Em muitos deles eh necessario inicializar todos os campos ponteiros tal 
 * como {NULL, NULL, NULL ...NULL}. See sim_init for initialization*/
...
/* Inicializações já estavam sendo feitas em sim_init_declarations() */
...
/* 2-level predictor config (<l1size> <l2size> <hist_size> <xor>) Wattch */
...
counter_t rename_access[MAX_SLOTS] = { 0 };
counter_t bpred_access[MAX_SLOTS] = { 0 };
counter_t window_access[MAX_SLOTS] = { 0 };
counter_t lsq_access[MAX_SLOTS] = { 0 };
counter_t regfile_access[MAX_SLOTS] = { 0 };
...
counter_t lsq_store_data_access[MAX_SLOTS] = { 0 };
counter_t lsq_load_data_access[MAX_SLOTS] = { 0 };
counter_t lsq_preg_access[MAX_SLOTS] = { 0 };
counter_t lsq_wakeup_access[MAX_SLOTS] = { 0 };

counter_t window_total_pop_count_cycle[MAX_SLOTS] = { 0 };
counter_t window_num_pop_count_cycle[MAX_SLOTS] = { 0 };
counter_t lsq_total_pop_count_cycle[MAX_SLOTS] = { 0 };
counter_t lsq_num_pop_count_cycle[MAX_SLOTS] = { 0 };
counter_t regfile_total_pop_count_cycle[MAX_SLOTS] = { 0 };
counter_t regfile_num_pop_count_cycle[MAX_SLOTS] = { 0 };
counter_t resultbus_total_pop_count_cycle[MAX_SLOTS] = { 0 };
counter_t resultbus_num_pop_count_cycle[MAX_SLOTS] = { 0 };
...
mem_access_latency(int blk_sz,
                   int sn) /* block size accessed, slot's number */
{
              ...
              int sn) /* slot's number to be accessed */
{
	...
	if (cache_dl2[0]) {
		...
		if ( (max_cycles) &&( (sim_cycle < 100) ||( (max_cycles - sim_cycle) < 100)))
			fprintf(stderr,"\ndl2 bank >> %d \n",dl2bank);

		/* CAUTION: Note that bsize from l1 must be <= bsize l2 */
		lat = cache_access(cache_dl2[dl2bank],
		                   cmd,
		                   baddr,
		                   NULL,
		                   bsize,
              ...
              int sn) /* slot's number */
{
	...
	if (cache_il2[0]) {
		...
		lat = cache_access(cache_il2[il2bank],
		                   cmd,
		                   baddr,
		                   NULL,
		                   bsize,
...
void sim_reg_options(struct opt_odb_t *odb) {
	...
	opt_reg_uint(odb,
	             "-il1banks:num",
	             "number of i-cache internal banks",
	             &il1_banks_num, /* default 1*/
	             il1_banks_num,
	             /* print */TRUE, /* format */
	             NULL,
	             0);
	...
	opt_reg_uint(odb,
	             "-dl1banks:num",
	             "number of d-cache banks",
	             &dl1_banks_num, /* default 1*/
	             dl1_banks_num,
	             /* print */TRUE, /* format */
	             NULL,
	             0);
	...
	opt_reg_int(odb,
	            ...
	            &ruu_fetch_width, /* default = decode_width*speed_fetch */
	            0,
	            /* print */TRUE, /* format */
	            NULL,
	            0);
	...
	opt_reg_string(odb,
	               ...
	               "l2 data cache config, i.e., {<config>|none}",
	               &cache_dl2_opt,
	               "ul2:1:256:64:4:l", /* 256K -> nsets = 1024 */
	               /* print */TRUE,
	               NULL,
	               0);
                       ...
                       char **argv) /* command line arguments */
{
	...
	if ( (il1_modules_num <= 0) || ((process_num % il1_modules_num) !=0) || (process_num < il1_modules_num) )
	...
	if (sscanf(pred_type, "%[^:]:%d",	name, &bpred_rate) == 2){
	...
	else if (!mystricmp(pred_type, "comb"))
	{
		...
		pred[sn] = bpred_create(BPredComb,
			...
			else /* dl1 is defined */
			{
				...
				if ( (dl1_banks_num <= 0) || ((process_num % dl1_banks_num) !=0) || (process_num < dl1_banks_num) )
				...
				/*
				 * csize: total size of cache in kB (kilo Bytes), modificado para passar o tamanho da cache e não o numero de conjuntos.
				 * Por isso que o nsets é calculado.
				 * */
				...
				else
				{
					...
					fatal(0,"bad l2 D-cache params: <name>:<nbanks>:<csize>:<bsize>:<assoc>:<repl>");
			...
			else /* il1 is defined */
			{
				...
				fatal(0,"bad l1 I-cache params:<name>:<nbanks>:<csize>:<bsize>:<assoc>:<repl>");
			...
			{	if (sscanf(itlb_opt, "%[^:]:%d:%d:%d:%c",
				...
				fatal(0,"bad TLB params: <name>:<csize>:<page_size>:<assoc>:<repl>",0);
...
void sim_reg_stats(struct stat_sdb_t *sdb) /* stats database */
{
	...
	for (sn=0; sn<process_num; sn++) {
		sprintf(stat_name,
		        "sim_num_stores_%s",
		        itoa[sn]);
		sprintf(formula_name,
		        "sim_num_refs_%s - sim_num_loads_%s",
		        itoa[sn],
		        itoa[sn]);
	...
	for (sn=0; sn<process_num; sn++) {
		sprintf(stat_name,
		        ...
		        itoa[sn]);
		sprintf(formula_name,
		        "sim_num_insn_%s / sim_elapsed_time",
		        itoa[sn]);
		stat_reg_formula(sdb,
		                 stat_name,
	...
	for (sn=0; sn<process_num; sn++) {
		sprintf(stat_name,
		        ...
		        itoa[sn]);
		sprintf(formula_name,
		        "sim_total_refs_%s - sim_total_loads_%s",
		        itoa[sn],
		        itoa[sn]);
		stat_reg_formula(sdb,
		                 stat_name,
	...
	for (sn=0; sn<process_num; sn++) {
		sprintf(stat_name,
		        ...
		        itoa[sn]);
		sprintf(formula_name,
		        "sim_num_insn_%s / sim_num_branches_%s",
		        itoa[sn],
		        itoa[sn]);
		stat_reg_formula(sdb,
		                 stat_name,
	...
	for (sn=0; sn<process_num; sn++) {
		sprintf(stat_name,
		        ...
		        itoa[sn]);
		sprintf(formula_name,
		        "sim_num_good_pred_%s / sim_num_branches_%s",
		        itoa[sn],
		        itoa[sn]);
		stat_reg_formula(sdb,
		                 stat_name,
	...
	if (cache_il1[0])
		for (bank=0; bank<IL1_BANKS_TOTAL; bank++)
			cache_single_reg_stats(cache_il1[bank],
			                       sdb);

	if (cache_il2[0])
		for (bank=0; bank<l2_banks_num; bank++)
			cache_single_reg_stats(cache_il2[bank],
			                       sdb);

	if (cache_dl1[0])
		for (bank=0; bank<dl1_banks_num; bank++)
			cache_single_reg_stats(cache_dl1[bank],
			                       sdb);

	if ( (cache_dl2[0]) && (!cache_dl1[0]))
		for (bank=0; bank<l2_banks_num; bank++)
			cache_single_reg_stats(cache_dl2[bank],
			                       sdb);

	if (itlb[0])
		cache_reg_stats(itlb,
		                sdb);

	if (dtlb[0])
		cache_reg_stats(dtlb,
		                sdb);
        ...
        /* register this stat */
        	pcstat_stats[sn][i] = stat;
					...
					sprintf(buf1,
					        "%s (by text address)",
					        stat->desc);
					pcstat_sdists[sn][i] = stat_reg_sdist(sdb,
					                                      buf,
					                                      buf1,
					                                      /* initial value */0,
...
#define MAX_RS_LINKS                    /* default */ 4096
      ...
      /* In this point, fname points to program name and argv has one 
         argument. Now, it identifies the arguments for the program */
				...
				if ( (new_argc >= 3) && (!strcmp(new_argv[new_argc-2], "<")))/* the input must be redirected*/{
   ...
   /* finish initialization of the simulation engine */
			...
			if (!mystricmp(fu_pool_type, "hetero")) {
				...
				fu_pool = res_create_pool(fu_pool_name,
				                          fu_hetero_config,
				                          N_ELT(fu_hetero_config),
				                          sn);
			} else {
					if (!mystricmp(fu_pool_type, "compact")) {
						...
						fu_pool = res_create_pool(fu_pool_name,
						                          fu_compact_config,
						                          N_ELT(fu_compact_config),
						                          sn);
					} else /* "homo" option */{
							strcpy(fu_pool_name, "fu_homo_pool");
							fu_pool = res_create_pool(fu_pool_name,
							                          fu_homo_config,
							                          N_ELT(fu_homo_config),
							                          sn);
	...
	((RS)->idep_ready[0] || (RS)->idep_ready[1])
				...
				static void ruu_dumpent(struct RUU_station *rs, /* ptr to RUU station */
					...
					myfprintf(sn,
					          ...
					          rs->pred_PC);
					fprintf(stream,
					        "         in_LSQ: %s, ea_comp: %s, recover_inst: %s\n",
					        rs->in_LSQ ? "t" : "f",
					        rs->ea_comp ? "t" : "f",
					        rs->recover_inst ? "t" : "f");
					myfprintf(sn,
					          stream,
					          "         spec_mode: %s, addr: 0x%08p, tag: 0x%08x\n",
					          rs->spec_mode ? "t" : "f",
					          rs->addr,
					          rs->tag);
					fprintf(stream,
					        "         seq: 0x%08x, ptrace_seq: 0x%08x\n",
					        rs->seq,
					        rs->ptrace_seq);
					fprintf(stream,
				...
				/* dump the contents of the RUU */
				static void ruu_dump(FILE *stream,
				                     int sn) /* output stream */
				{
					int num, head;
...
;
				...
				/* dump the contents of the RUU */
				static void lsq_dump(FILE *stream,
				                     int sn) /* output stream */
				{
					int num, head;
					struct RUU_station *rs;
...
#define RSLINK_FREE_LIST(LINK)						\
  {  struct RS_link *fl_link, *fl_link_next;				\
	 ...
	 RSLINK_FREE(fl_link);						\
				...
				static void rslink_init(int nlinks, /* total number of RS_LINK available */
				                        int sn) /* slot's number */
				{
					int i;
					struct RS_link *link;
...
;
				...
				static void eventq_dump(FILE *stream,
				                        int sn) /* output stream */
				{
					struct RS_link *ev;
					...
					for (ev = event_queue[sn]; ev != NULL; ev = ev->next) {
						...
						if (RSLINK_VALID(ev)) {
							...
							fprintf(stream,
							        "idx: %2d: @ %.0f\n",
							        (int)(rs - (rs->in_LSQ ? LSQ[sn] : RUU[sn])),
							        (double)ev->x.when);
							ruu_dumpent(rs,
							            rs - (rs->in_LSQ ? LSQ[sn] : RUU[sn]),
							            stream, /* !header */
							            FALSE,
							            sn);
						}
				...
				static void eventq_queue_event(struct RUU_station *rs,
				                               tick_t when,
				                               int sn) {
					...
					for (prev=NULL,
					     ev=event_queue[sn]; ev && ev->x.when < when; prev=ev,
					                                                  ev=ev->next)
						;
				...
				static struct RUU_station *eventq_next_event(int sn) {
					struct RS_link *ev;
				...
				static void readyq_dump(FILE *stream,
				                        int sn) /* output stream */
				{
					struct RS_link *link;
				...
				static void readyq_enqueue(struct RUU_station *rs,
				                           int sn) /* RS to enqueue */
				{
...
;
					...
					if (rs->in_LSQ || MD_OP_FLAGS(rs->op) & (F_LONGLAT|F_CTRL)) {
						...
						for (prev=NULL,
						     node=ready_queue[sn]; node && node->x.seq < rs->seq; prev=node,
						                                                          node=node->next)
							;
					}
...
#define SET_CREATE_VECTOR(N,L)  (spec_mode[sn]                             \
				...
				static void cv_dump(FILE *stream,
				                    int sn) /* output stream */
				{
					int i;
					struct CV_link ent;
					...
					for (i=0; i < MD_TOTAL_REGS; i++) {
						...
						if (!ent.rs)
							fprintf(stream,
							        "[cv%02d]: from architected reg file\n",
							        i);
						else
							fprintf(stream,
							        "[cv%02d]: from %s, idx: %d\n",
							        i,
							        (ent.rs->in_LSQ ? "LSQ" : "RUU"),
...
static void ruu_commit(void) {
	...
	sn = next_slot;
	while ((not_done) && (n_committed < ruu_commit_width)) {
			...
			if (!rs->completed) {
				...
				if (RUU[sn][RUU_head[sn]].ea_comp) {
					...
					if (!LSQ[sn][LSQ_head[sn]].completed){
						...
						if ((MD_OP_FLAGS(LSQ[sn][LSQ_head[sn]].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE)){
							...
							if (fu) {
								...
								if (cache_dl1[0]) {
									...
									/* Find the dl1bank of atual process by slot number */
									dl1bank = WHICHIS_DL1BANK_OF_SLOT(sn);
									...
									lat = cache_access(cache_dl1[dl1bank], Write, addr, NULL, 4, sim_cycle, NULL, NULL,sn);
									...
									/* Find the dl2bank of atual process by slot number */
									dl2bank = WHICHIS_DL2BANK_OF_DL1(dl1bank);
								...
								if (dtlb[0]) {
									/* access the D-TLB */
									lat = cache_access(dtlb[sn], Read, addr, NULL, 4, sim_cycle, NULL, NULL,sn);
									if (lat > 1)
										events |= PEV_TLBMISS;
								}
							}
				...
				if (!broke){
					...
					if ((MD_OP_FLAGS(rs->op) & (F_ICOMP|F_FCOMP)) || ((MD_OP_FLAGS(rs->op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD))) {
						regfile_access[sn]++;
				#ifdef DYNAMIC_AF	
						regfile_total_pop_count_cycle[sn] += pop_count(rs->val_rc);
					...
					if (pred[sn] && bpred_spec_update == spec_CT && (MD_OP_FLAGS(rs->op) & F_CTRL)) {
...
*/
static void ruu_recover(int branch_index,
                        int sn) /* index of mis-pred branch */
{
	...
	for (sn=0; sn<process_num; sn++) {
		...
		while ((rs = eventq_next_event(sn))) {
			...
			if (!(MD_OP_FLAGS(rs->op) & F_CTRL)) {
				...
				// fprintf(stdout,"writeback: window_total_pop_count_cycle: %g\n", window_total_pop_count_cycle[process_instance]);
				// fprintf(stdout,"writeback: window_num_pop_count_cycle: %g\n", window_num_pop_count_cycle[process_instance]);
						
				resultbus_total_pop_count_cycle[sn] += pop_count(rs->val_rc);
				resultbus_num_pop_count_cycle[sn]++;
						
				// fprintf(stdout,"writeback: resultbus_total_pop_count_cycle: %g\n", resultbus_total_pop_count_cycle[process_instance]);
				// fprintf(stdout,"writeback: resultbus_num_pop_count_cycle: %g\n", resultbus_num_pop_count_cycle[process_instance]);
			...
			if (rs->recover_inst) {
				...
				bpred_recover(pred[sn],
				              rs->PC,
				              rs->stack_recover_idx);
...
static void lsq_refresh(void) {
	...
	for (sn=0; sn<process_num; sn++) {
		...
		for (i=0,
		     index=LSQ_head[sn],
		     n_std_unknowns=0; i < LSQ_num[sn]; i++,
...
static void ruu_issue(void) {
	...
	while ((not_done) && (n_issued < ruu_issue_width)) {
		/* to restore the error integrity */
		errno = aux_errno[sn];
		...
		if ((node[sn]) && (!issued[sn])) {
			...
			if (RSLINK_VALID(node[sn])) {
				...
				if (rs->in_LSQ&& ((MD_OP_FLAGS(rs->op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))) {
					...
					lsq_preg_access[sn]++;
				#ifdef DYNAMIC_AF	
					lsq_total_pop_count_cycle[sn] += pop_count(rs->val_ra);
					lsq_num_pop_count_cycle[sn]++;
				#endif
						...
						if (fu) {
							...
							if (rs->in_LSQ&& ((MD_OP_FLAGS(rs->op) & (F_MEM|F_LOAD))== (F_MEM|F_LOAD))) {
								...
								if (i != LSQ_head[sn]) {
									...
									#ifdef DYNAMIC_AF	
											lsq_total_pop_count_cycle[sn] += pop_count(rs->val_ra_result);
											lsq_num_pop_count_cycle[sn]++;
									#endif
											break;
										}
								...
								if (!load_lat)
								{
									...
									if (cache_dl1[0] /* there is dl1 */ && valid_addr)
									{	
										/* Wattch -- D-cache access */
										dcache_access[sn]++;
										...
										load_lat = cache_access(cache_dl1[dl1bank], Read, addr, NULL, 4, sim_cycle, NULL, NULL,sn);
								...
								if (dtlb[0] && MD_VALID_ADDR(rs->addr)){
									...
									tlb_lat =	cache_access(dtlb[sn], Read, addr, NULL, 4, sim_cycle, NULL, NULL,sn);
							...
							window_preg_access[sn]++;
						#ifdef DYNAMIC_AF	
							window_total_pop_count_cycle[sn] += pop_count(rs->val_ra) + pop_count(rs->val_rb);
							window_num_pop_count_cycle[sn] += 2;
															
							// fprintf(stdout,"issue: window_total_pop_count_cycle: %g\n", window_total_pop_count_cycle[process_instance]);
							// fprintf(stdout,"issue: window_num_pop_count_cycle: %g\n", window_num_pop_count_cycle[process_instance]);
						#endif
...
static void rspec_dump(FILE *stream, /* output stream */
                       int sn) /* slot's number */
{
	int i;
...
static void tracer_recover(int sn) /* slot's number */
{
	int i;
...
static void tracer_init(int sn) /* slot's number */
{
	int i;
                                          ...
                                          int sn) /* slot's number */
{
	...
	/* check permissions */
	if (!((addr >= ld_text_base[sn]&& addr < (ld_text_base[sn]+ld_text_size[sn])&& cmd == Read)|| MD_VALID_ADDR(addr))) {
		/* no can do, return zero result */
		for (i=0; i < nbytes; i++)
			((char *)p)[i] = 0;
	...
	for (prev=NULL,
	     ent=store_htable[sn][index]; ent; prev=ent,
	                                       ent=ent->next) {
		if (ent->addr == addr) {
                                 ...
                                 int sn) /* slot's number */
{
	...
	#ifdef DYNAMIC_AF	
		if (idep_num == 0)
			regfile_total_pop_count_cycle[sn] += pop_count(rs->val_ra);
		else
			regfile_total_pop_count_cycle[sn] += pop_count(rs->val_rb);
		regfile_num_pop_count_cycle[sn]++;
				
		// fprintf(stdout,"link_idep: regfile_total_pop_count_cycle: %g\n", regfile_total_pop_count_cycle[process_instance]);
		// fprintf(stdout,"link_idep: regfile_num_pop_count_cycle: %g\n", regfile_num_pop_count_cycle[process_instance]);
	#endif
	...
	switch (rt) {
		...
		case rt_PC:
			if (!is_write) {
				...
				regs[sn]->regs_PC = eval_as_addr(*val,
				                                 sn);
			break;
		...
		case rt_NPC:
			if (!is_write) {
				...
				regs[sn]->regs_NPC = eval_as_addr(*val,
				                                  sn);
			break;
...
static void ruu_dispatch(void) {
	...
	while ( (not_done) &&(n_dispatched < (ruu_decode_width * fetch_speed))&&(ruutotal() < ruu_totalsize)&&(lsqtotal() < lsq_totalsize)) {
		/* to restore the error integrity */
		errno = aux_errno[sn];
		...
		&& (ruu_include_spec || !spec_mode[sn])) {
			...
			if (ruu_inorder_issue&& (last_op[sn].rs && RSLINK_VALID(&last_op[sn])&& !OPERANDS_READY(last_op[sn].rs))) {
...
#include "machine.def"
				...
				if (fault != md_fault_none)
					fatal(sn,"non-speculative fault (%d) detected @ 0x%08p", fault, regs[sn]->regs_PC);
				...
				if (op != MD_NOP_OP)
				{
					...
					if (MD_OP_FLAGS(op) & F_MEM)
					{
						...
						if (OPERANDS_READY(rs))
						{
							...
							regfile_total_pop_count_cycle[sn] += pop_count(rs->val_ra);
							regfile_total_pop_count_cycle[sn] += pop_count(rs->val_rb);
							regfile_num_pop_count_cycle[sn] +=2;
				...
				ptrace_newstage(pseq, PST_DISPATCH, (pred_PC[sn] != regs[sn]->regs_NPC) ? PEV_MPOCCURED : 0);
                ...
                int sn) /* slot's number */
{
	...
	myfprintf(sn,
	          stream,
	          "pred_PC: 0x%08p, recover_PC: 0x%08p\n",
	          pred_PC[sn],
	          recover_PC[sn]);
	myfprintf(sn,
	          stream,
	...
	while (num) {
		fprintf(stream,
		        "idx: %2d: inst: `",
		        head);
		md_print_insn(fetch_data[sn][head].IR,
		              fetch_data[sn][head].regs_PC,
		              stream);
		fprintf(stream,
		        "'\n");
		myfprintf(sn,
		          stream,
		          "         regs_PC: 0x%08p, pred_PC: 0x%08p\n",
		          fetch_data[sn][head].regs_PC,
		          fetch_data[sn][head].pred_PC);
...
static void ruu_fetch(int sn,
                      int il1bank) /* slot's number, i-cache bank */
{
	...
	for (i=0,
		...
		if (ld_text_base[sn] <= fetch_regs_PC[sn]&& fetch_regs_PC[sn] < (ld_text_base[sn]+ld_text_size[sn])&& !(fetch_regs_PC[sn] & (sizeof(md_inst_t)-1))) {
			md_addr_t addr;
		...
		if (pred[sn]) {
			...
			if (MD_OP_FLAGS(op) & F_CTRL)
				fetch_pred_PC[sn] =bpred_lookup(pred[sn],
...
;		} /* end of switch (op) */
		...
		/* check for DLite debugger entry condition */
		if (dlite_check_break(regs[sn]->regs_NPC, is_write ? ACCESS_WRITE : ACCESS_READ, addr, sim_num_insn[sn], sim_num_insn[sn]))
	...
	for (module=0; module<il1_modules_num; module++) {
		...
		if ((max_cycles) && ((sim_cycle < 100) || ((max_cycles - sim_cycle) < 100))) {
			...
			for (i=0; i<IL1_MODULE_WIDTH; i++)
				fprintf(stderr," %d (%d)", fetch_num[i + FIRST_SLOT_OF_MOD(module)], ruu_slot_delay[i + FIRST_SLOT_OF_MOD(module)]);
		...
		if (slot_scheduled)	{
			...
			if ((sim_cycle > 1000) && (BOUND_POS(cache_il1[il1bank]->bus_free - sim_cycle) > max_bank_delay))	{
...
void sim_main(void) {
	...
	for (sn=0; sn<process_num; sn++)
		if (dlite_check_break(regs[sn]->regs_PC, /*no access*/0, /*addr*/0, 0, 0))
			dlite_main(regs[sn]->regs_PC,
			           regs[sn]->regs_PC + sizeof(md_inst_t),
			           sim_cycle,
			           regs[sn],
			           mem[sn],
			           sn);
	...
	if ((exit_code = setjmp(sim_exit_buf)) != 0) { /* special handling as longjmp cannot pass 0 
