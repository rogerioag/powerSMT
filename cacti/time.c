	...
	int stack; /* number of transistors in series that are on */
{
	...
	if (width >= 10) {
		cap = 3.0*Leff*width/2.0*Cdiffarea + 6.0*Leff*Cdiffside + width*Coverlap;
		cap += (double)(stack-1)*(Leff*width*Cdiffarea + 4.0*Leff*Cdiffside + 2.0*width*Coverlap);
	} else {
		cap = 3.0*Leff*width*Cdiffarea + (6.0*Leff+width)*Cdiffside + width*Coverlap;
		cap += (double)(stack-1)*(Leff*width*Cdiffarea + 2.0*Leff*Cdiffside + 2.0*width*Coverlap);
	}
	return (cap);
}
...
double decoder_delay(C, B, A, Ndwl, Ndbl, Nspd, Ntwl, Ntbl, Ntspd, Tdecdrive, Tdecoder1, Tdecoder2, outrisetime)
	int C, B, A, Ndwl, Ndbl, Nspd, Ntwl, Ntbl, Ntspd;double *Tdecdrive, *Tdecoder1, *Tdecoder2, *outrisetime; {
	...
	Ceq = draincap(Wdecdrivep,PCH,1)+draincap(Wdecdriven,NCH,1) +gatecap(Wdecdrivep+Wdecdriven,0.0);
	tf = Ceq*transreson(Wdecdriven,NCH,1);
	nextinputtime = horowitz(	nextinputtime,
														tf,
														VTHINV100x60,
														VTHINV100x60,
														RISE)/(1.0-VTHINV100x60);

	/* First stage: driving the decoders */

	rows = C/(8*B*A*Ndbl*Nspd);
	Ceq = draincap(Wdecdrivep,PCH,1)+draincap(Wdecdriven,NCH,1) +4*gatecap(Wdec3to8n+Wdec3to8p,10.0)*(Ndwl*Ndbl)+Cwordmetal*0.25*8*B*A*Ndbl*Nspd;
	Rwire = Rwordmetal*0.125*8*B*A*Ndbl*Nspd;
	tf = (Rwire + transreson(Wdecdrivep,PCH,1))*Ceq;
	*Tdecdrive = horowitz(nextinputtime,
												tf,
												VTHINV100x60,
												VTHNAND60x90,
												FALL);
	...
	numstack =ceil((1.0/3.0)*logtwo( (double)((double)C/(double)(B*A*Ndbl*Nspd))));
	if (numstack==0)
		numstack = 1;
	if (numstack>5)
		numstack = 5;
	Ceq = 3*draincap(Wdec3to8p,PCH,1) +draincap(Wdec3to8n,NCH,3) +gatecap(WdecNORn+WdecNORp,((numstack*40)+20.0))*rows +Cbitmetal*rows*8;
	...
	*Tdecoder1 = horowitz(nextinputtime,
												tf,
												VTHNAND60x90,
												vth,
												RISE);
	...
	tf = Req*Ceq;
	*Tdecoder2 = horowitz(nextinputtime,
												tf,
												vth,
												VSINV,
												FALL);
...
double decoder_tag_delay(C, B, A, Ndwl, Ndbl, Nspd, Ntwl, Ntbl, Ntspd, Tdecdrive, Tdecoder1, Tdecoder2, outrisetime)
	int C, B, A, Ndwl, Ndbl, Nspd, Ntwl, Ntbl, Ntspd;double *Tdecdrive, *Tdecoder1, *Tdecoder2, *outrisetime; {
	double Ceq, Req, Rwire, rows, tf, nextinputtime, vth, tstep, m, a, b, c;
	int numstack;
	...
	Ceq = draincap(Wdecdrivep,PCH,1)+draincap(Wdecdriven,NCH,1) +4*gatecap(Wdec3to8n+Wdec3to8p,10.0)*(Ntwl*Ntbl)+Cwordmetal*0.25*8*B*A*Ntbl*Ntspd;
	...
	*Tdecdrive = horowitz(nextinputtime,
	...
	numstack =ceil((1.0/3.0)*logtwo( (double)((double)C/(double)(B*A*Ntbl*Ntspd))));
	if (numstack==0)
		numstack = 1;
	if (numstack>5)
		numstack = 5;
...
double wordline_delay(B, A, Ndwl, Nspd, inrisetime, outrisetime)
	int B, A, Ndwl, Nspd;double inrisetime, *outrisetime; {
	...
	Cline = (gatecappass(Wmemcella,0.0)+gatecappass(Wmemcella,0.0)+Cwordmetal)*cols;
	...
	Ceq = draincap(Wdecinvn,NCH,1) + draincap(Wdecinvp,PCH,1) +gatecap(	nsize+psize,
																																			20.0);
	tf = transreson(Wdecinvn,NCH,1)*Ceq;

	Tworddrivedel = horowitz(	inrisetime,
														tf,
														VSINV,
														VSINV,
														RISE);
	...
	Cline = (gatecappass(Wmemcella,(BitWidth-2*Wmemcella)/2.0)+gatecappass(Wmemcella,(BitWidth-2*Wmemcella)/2.0)+Cwordmetal)*cols+draincap(	nsize,
																														NCH,
																														1) + draincap(psize,
																																					PCH,
																																					1);
	...
	tf = (transreson(	psize,
										PCH,
										1)+Rline)*Cline;
	Twordchargedel = horowitz(nextinputtime,
														tf,
														VSINV,
														VSINV,
														FALL);
...
double wordline_tag_delay(C, A, Ntspd, Ntwl, inrisetime, outrisetime)
	int C, A, Ntspd, Ntwl;double *outrisetime;double inrisetime; {
	double tf, m, a, b, c;
	...
	Ceq = draincap(Wdecinvn,NCH,1) + draincap(Wdecinvp,PCH,1) +gatecap(Wdecinvn+Wdecinvp,20.0);
	tf = transreson(Wdecinvn,NCH,1)*Ceq;
	...
	/* second stage */
	Cline = (gatecappass(Wmemcella,(BitWidth-2*Wmemcella)/2.0)+gatecappass(Wmemcella,(BitWidth-2*Wmemcella)/2.0)+Cwordmetal)*tagbits*A*Ntspd/Ntwl+draincap(Wdecinvn,NCH,1) + draincap(Wdecinvp,PCH,1);
...
double bitline_delay(C, A, B, Ndwl, Ndbl, Nspd, inrisetime, outrisetime)
	int C, A, B, Ndbl, Nspd;double inrisetime, *outrisetime; {
	...
	if (Ndbl*Nspd == 1) {
		...
		Cline = rows*(Cbitrow+Cbitmetal)+ 2*draincap(Wbitpreequ,PCH,1) +draincap(Wbitmuxn,NCH,1);
		Ccolmux = Nspd*Ndbl*(draincap(Wbitmuxn,NCH,1))+2*gatecap(WsenseQ1to4,10.0);
		Rlineb = Rbitmetal*rows/2.0;
		r1 = Rlineb +transreson(Wbitmuxn,NCH,1);
	}
	r2 = transreson(Wmemcella,NCH,1) +transreson(Wmemcella*Wmemcellbscale,NCH,1);
	c1 = Ccolmux;
	c2 = Cline;
...
double bitline_tag_delay(C, A, B, Ntwl, Ntbl, Ntspd, inrisetime, outrisetime)
	int C, A, B, Ntbl, Ntspd;double inrisetime, *outrisetime; {
	double Tbit, Cline, Ccolmux, Rlineb, r1, r2, c1, c2, a, b, c;
	double m, tstep;
	double Cbitrow; /* bitline capacitance due to access transistor */
	int rows, cols;
...
double compare_time(C, A, Ntbl, Ntspd, inputtime, outputtime)
	int C, A, Ntbl, Ntspd;double inputtime, *outputtime; {
	...
	Ceq = gatecap(Wcompinvn2+Wcompinvp2,10.0) +draincap(Wcompinvp1,PCH,1) +draincap(Wcompinvn1,NCH,1);

	Req = transreson(Wcompinvp1,PCH,1);
	tf = Req*Ceq;
	st1del = horowitz(inputtime,
										tf,
										VTHCOMPINV,
										VTHCOMPINV,
										FALL);
	...
	Ceq = gatecap(Wcompinvn3+Wcompinvp3,10.0) +draincap(Wcompinvp2,PCH,1) +draincap(Wcompinvn2,NCH,1);

	Req = transreson(Wcompinvn2,NCH,1);
	tf = Req*Ceq;
	st2del = horowitz(inputtime,
										tf,
										VTHCOMPINV,
										VTHCOMPINV,
										RISE);
	...
	Ceq = gatecap(Wevalinvn+Wevalinvp,10.0) +draincap(Wcompinvp3,PCH,1) +draincap(Wcompinvn3,NCH,1);

	Req = transreson(Wcompinvp3,PCH,1);
	tf = Req*Ceq;
	st3del = horowitz(nextinputtime,
										tf,
										VTHCOMPINV,
										VTHEVALINV,
										FALL);
	...
	c2 = (tagbits)*(draincap(Wcompn,NCH,1)+draincap(Wcompn,NCH,2))+draincap(Wevalinvp,PCH,1) + draincap(Wevalinvn,NCH,1);
	c1 = (tagbits)*(draincap(Wcompn,NCH,1)+draincap(Wcompn,NCH,2))+draincap(Wcompp,PCH,1) + gatecap(Wmuxdrv12n+Wmuxdrv12p,20.0) +cols*Cwordmetal;
...
double mux_driver_delay(C, B, A, Ndbl, Nspd, Ndwl, Ntbl, Ntspd, inputtime, outputtime)
	int C, B, A, Ndbl, Nspd, Ndwl, Ntbl, Ntspd;double inputtime, *outputtime; {
	...
	Ceq = gatecap(WmuxdrvNORn+WmuxdrvNORp,15.0)*(8*B/BITOUT)+draincap(Wmuxdrv12n,NCH,1) + draincap(Wmuxdrv12p,PCH,1);
	Req = transreson(Wmuxdrv12p,PCH,1);
	tf = Ceq*Req;
	Tst1 = horowitz(inputtime,
									tf,
									VTHMUXDRV1,
									VTHMUXDRV2,
									FALL);
	...
	Ceq = gatecap(Wmuxdrv3n+Wmuxdrv3p,15.0) + 2*draincap(WmuxdrvNORn,NCH,1) +draincap(WmuxdrvNORp,PCH,2);
	Req = transreson(WmuxdrvNORn,NCH,1);
	tf = Ceq*Req;
	Tst2 = horowitz(nextinputtime,
									tf,
									VTHMUXDRV2,
									VTHMUXDRV3,
									RISE);
...
double valid_driver_delay(C, A, Ntbl, Ntspd, inputtime)
	int C, A, Ntbl, Ntspd;double inputtime; {
	double Ceq, Tst1, tf;
...
double dataoutput_delay(C, B, A, Ndbl, Nspd, Ndwl, inrisetime, outrisetime)
	int C, B, A, Ndbl, Nspd, Ndwl;double *outrisetime, inrisetime; {
	...
	Ceq = 2*draincap(Woutdrvnorn,NCH,1)+draincap(Woutdrvnorp,PCH,2)+gatecap(Woutdrivern,10.0);
	tf = Ceq*transreson(Woutdrvnorp,PCH,2);
	nordel = horowitz(inrisetime,
										tf,
										VTHOUTDRNOR,
										VTHOUTDRIVE,
										FALL);
	...
	Ceq = (draincap(Woutdrivern,NCH,1)+draincap(Woutdriverp,PCH,1))*((8*B*A)/BITOUT)+Cwordmetal*(8*B*A*Nspd* (vstack))+ Cout;
	Rwire = Rwordmetal*(8*B*A*Nspd* (vstack))/2;

	tf = Ceq*(transreson(Woutdriverp,PCH,1)+Rwire);
	outdel = horowitz(nextinputtime,
										tf,
										VTHOUTDRIVE,
										0.5,
										RISE);
...
double selb_delay_tag_path(inrisetime, outrisetime)
	...
	Ceq = draincap(Woutdrvseln,NCH,1)+draincap(Woutdrvselp,PCH,1)+gatecap(Woutdrvnandn+Woutdrvnandp,10.0);
	tf = Ceq*transreson(Woutdrvseln,NCH,1);
	Tst1 = horowitz(inrisetime,
									tf,
									VTHOUTDRINV,
									VTHOUTDRNAND,
									RISE);
...
double precharge_delay(worddata)
	...
	Ceq = draincap(Wdecinvn,NCH,1)+draincap(Wdecinvp,PCH,1)+4*gatecap(Wdecinvn+Wdecinvp,0.0);
	tf = Ceq*transreson(Wdecinvn,NCH,1);
	pretime = 4*horowitz(	0.0,
												tf,
												0.5,
												0.5,
												RISE) + worddata;
...
void calculate_time(result, parameters)
	result_type *result;parameter_type *parameters; {
				...
				for (Ntspd=1; Ntspd<=MAXSPD; Ntspd=Ntspd*2) {
					for (Ntwl=1; Ntwl<=1; Ntwl=Ntwl*2) {
						for (Ntbl=1; Ntbl<=MAXN; Ntbl=Ntbl*2) {
							...
							if (organizational_parameters_valid(rows,
																									columns,
																									Ndwl,
																									Ndbl,
																									Nspd,
								...
								wordline_data = wordline_delay(	parameters->block_size,
																								...
																								&outrisetime);
								inrisetime = outrisetime;
								bitline_data = bitline_delay(	parameters->cache_size,
																							parameters->associativity,
																							parameters->block_size,
																							Ndwl,
																							Ndbl,
																							...
																							&outrisetime);
								inrisetime = outrisetime;
								sense_amp_data = sense_amp_delay(	inrisetime,
																									&outrisetime);
								inrisetime = outrisetime;
								data_output = dataoutput_delay(	parameters->cache_size,
																								parameters->block_size,
																								parameters->associativity,
																								Ndbl,
																								Nspd,
								...
								if (parameters->associativity==1) {
									before_mux = decoder_data + wordline_data + bitline_data +sense_amp_data + data_output;
									after_mux = 0;
								} else {
									before_mux = decoder_data + wordline_data + bitline_data +sense_amp_data;
								...
								bitline_tag = bitline_tag_delay(parameters->cache_size,
																								parameters->associativity,
																								parameters->block_size,
																								Ntwl,
																								Ntbl,
								...
								sense_amp_tag = sense_amp_tag_delay(inrisetime,
																										&outrisetime);
								inrisetime = outrisetime;

								compare_tag = compare_time(	parameters->cache_size,
																						parameters->associativity,
																						Ntbl,
																						Ntspd,
																						inrisetime,
								...
								if (parameters->associativity == 1) {
									...
									valid_driver = valid_driver_delay(parameters->cache_size,
																										...
																										Ntspd,
																										inrisetime);
									time_till_compare = decoder_tag + wordline_tag + bitline_tag +sense_amp_tag;
									...
									mux_driver = mux_driver_delay(parameters->cache_size,
																								parameters->block_size,
																								parameters->associativity,
																								Ndbl,
																								Nspd,
