...
int start_command_line, last_command_line;
...
static void usage(FILE *fd, int argc, char **argv) {
	fprintf(fd,
					"Usage: %s <x1> <x2> {-options} <batch file>\n\n",
					argv[0]);
...
int main(int argc, char **argv, char **envp) {
	...
	/* from difference between the start and finish lines. */
	{
		...
		if ( !start_command_line ||
			...
			/* To initialize all global variables from all modules, because after the replication 
			 * they din't can be initialized on their declaration.*/
			...
			opt_reg_flag(	sim_odb,
										...
										NULL,
										0);
			opt_reg_string(	sim_odb,
											"-chkpt",
											"restore EIO trace execution from <fname>",
											&sim_chkpt_fname, /* default */
											NULL, /* !print */
											FALSE,
											NULL,
											0);
			...
			opt_reg_int(sim_odb,
									...
									"simulator scheduling priority",
									&nice_priority,
									/* default */NICE_DEFAULT_VALUE, /* print */
									TRUE,
									NULL,
									0);
#endif
